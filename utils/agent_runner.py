...
def _strip_thinking(text: str) -> str:
    """
    Strip AI thinking/reasoning text from generated content.
    Uses AI (gemini-2.0-flash) for semantic cleaning — works for all languages.
    Falls back to basic pattern matching if AI call fails.
    """
    stripped = text.strip()
    if not stripped:
        return text

    # Try AI-based cleaning (works for both CJK and English)
    try:
        cleaned = _ai_clean_thinking(stripped)
        if cleaned and len(cleaned) > len(stripped) * 0.3:
            stripped = cleaned
            logger.info(f"AI cleaning: {len(text)} -> {len(stripped)} chars")
        else:
            logger.warning("AI cleaning returned too little content, falling back to pattern matching")
            stripped = _basic_strip_thinking(stripped)
    except Exception as e:
        logger.warning(f"AI cleaning failed ({e}), falling back to pattern matching")
        stripped = _basic_strip_thinking(stripped)

    # Remove trailing duplicate heading sequences
    stripped = _remove_trailing_duplicates(stripped)

    return stripped.strip()


def _ai_clean_thinking(text: str) -> str:
    """
    Use gemini-2.0-flash to semantically remove AI thinking/planning text.
    Works for any language — the AI understands content vs meta-commentary.
    Note: genai.configure() is already called in setup_model(), no need to repeat here.
    """
    cleaner_model = genai.GenerativeModel(
        "gemini-2.0-flash",
    ...
    cleaning_prompt = (
        "You are a text cleaner for academic papers. The text below was generated by an AI, "
        "and it may contain the AI's internal thinking, planning, self-talk, or meta-commentary "
        "mixed in with the actual academic content.\n\n"
        "**Your task:** Output ONLY the actual academic paper content.\n\n"
        "**Remove ALL of the following:**\n"
        "- AI thinking/planning (\"I will write...\", \"Let me...\", \"I need to...\", \"我需要...\", \"让我...\")\n"
        "- Meta-commentary (\"The word count is...\", \"This section should...\", \"字数要求...\")\n"
        "- Self-referential text (\"My approach...\", \"I'll structure...\", \"我的策略...\")\n"
        "- Draft notes, strategies, outlines that are NOT part of the paper itself\n"
        "- Summaries of instructions or task descriptions\n"
        "- Code blocks containing planning notes\n"
        "- Any text where the AI talks about itself or the writing process\n\n"
        "**STRICT RULES:**\n"
        "1. Keep ALL academic prose, headings, citations, tables, lists, figures — unchanged\n"
        "2. Keep the EXACT formatting (markdown, bold, italic, etc.)\n"
        "3. Do NOT rewrite, rephrase, translate, or improve ANY content\n"
        "4. Do NOT add any text of your own — no explanations, no headers, no notes\n"
        "5. If the text has no thinking content, return it completely unchanged\n"
        "6. Preserve the original language of the academic content\n\n"
        "Output the cleaned text directly:\n\n"
        f"{text}"
    )

    response = cleaner_model.generate_content(cleaning_prompt)
...
def _basic_strip_thinking(text: str) -> str:
    ...
    thinking_patterns = [
        ...
        r'^\*\*(?:My |Planning|Note|Draft|Approach|Strategy|Abstract|Research)',
        r'^(?:The (?:task|goal|structure|word count|core|key|draft|section|final))',
        r'^(?:First|Then|Next|Finally|Also|That\'s|So |This is where )',
        r'^(?:```)',
    ]
...
def run_agent(
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # Strip AI thinking/reasoning text that appears before actual content
            # The model sometimes outputs planning text before the markdown content
...
def research_citations_via_api(
    ...
    # Early stopping at 25 citations (fast turnaround)
